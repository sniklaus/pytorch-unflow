...
kernel_Correlation_updateGradFirst = '''
	...
	extern "C" __global__ void kernel_Correlation_updateGradFirst(
	  ...
	  int l = (intIndex / SIZE_1(gradFirst)) % SIZE_3(gradFirst) + 20; // w-pos
	  int m = (intIndex / SIZE_1(gradFirst) / SIZE_3(gradFirst)) % SIZE_2(gradFirst) + 20; // h-pos
	  ...
	  const int bot0index = ((n * SIZE_2(gradFirst)) + (m-20)) * SIZE_3(gradFirst) + (l-20);
...
kernel_Correlation_updateGradSecond = '''
	...
	extern "C" __global__ void kernel_Correlation_updateGradSecond(
	  ...
	  int l = (intIndex / SIZE_1(gradSecond)) % SIZE_3(gradSecond) + 20; // w-pos
	  int m = (intIndex / SIZE_1(gradSecond) / SIZE_3(gradSecond)) % SIZE_2(gradSecond) + 20; // h-pos
	  ...
	  const int bot1index = ((n * SIZE_2(gradSecond)) + (m-20)) * SIZE_3(gradSecond) + (l-20);
...
class _FunctionCorrelation(torch.autograd.Function):
	...
	def forward(self, first, second):
		rbot0 = first.new_zeros([ first.shape[0], first.shape[2] + 40, first.shape[3] + 40, first.shape[1] ])
		rbot1 = first.new_zeros([ first.shape[0], first.shape[2] + 40, first.shape[3] + 40, first.shape[1] ])
		...
		output = first.new_zeros([ first.shape[0], 441, first.shape[2], first.shape[3] ])
