...
class _FunctionCorrelation(torch.autograd.Function):
	...
	def forward(self, first, second):
		rbot0 = first.new_zeros([ first.size(0), first.size(2) + 40, first.size(3) + 40, first.size(1) ])
		rbot1 = first.new_zeros([ first.size(0), first.size(2) + 40, first.size(3) + 40, first.size(1) ])
